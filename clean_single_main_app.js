...
// Find the index of `// ================= MAIN APPLICATION =================`
const mainAppIndex = appCode.indexOf('// ================= MAIN APPLICATION =================');
if (mainAppIndex !== -1) {
  // Find the header tag inside MainApp return JSX: `<header className="max-w-3xl mx-auto mb-4 font-arabic">`
  const headerIndex = appCode.indexOf('<header className="max-w-3xl mx-auto mb-4 font-arabic">', mainAppIndex);
  
  if (headerIndex !== -1) {
    const cleanMainAppTop = `// ================= MAIN APPLICATION =================
function MainApp() {
...
`;
    ...
    appCode = appCode.slice(0, mainAppIndex) + cleanMainAppTop + appCode.slice(headerIndex);
    fs.writeFileSync('e:\\ali-Gym-Track\\src\\App.jsx', appCode, 'utf8');
    console.log('Successfully cleaned single MainApp definition!');
  }
}
