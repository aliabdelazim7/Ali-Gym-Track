...
async function run() {
  console.log('Verifying & downloading all 22 exercises offline...');
  let fixedCount = 0;
    ...
    for (let frame = 0; frame <= 1; frame++) {
      ...
      if (!exists) {
        ...
        try {
          ...
          console.log(`✓ FIXED & Downloaded ${fileName} (${ex.name})`);
          fixedCount++;
        } catch (err) {
  ...
  console.log(`All files checked! Fixed ${fixedCount} missing files.`);
}
